...
interface DirectionalAudio {
  id: string;
  label: string | null;
  direction: string;
  distance_trigger_m: number;
  audio_url: string;
...
const schema = z.object({
  ...
  distance_trigger_m: z.coerce.number().min(0, 'Distance required'),
...
const DIRECTIONS = ['left', 'right', 'straight', 'turn_around'];

export default function DirectionalPage() {
  const { ready } = useManageAuth();
  const [items, setItems] = useState<DirectionalAudio[]>([]);
  const [filtered, setFiltered] = useState<DirectionalAudio[]>([]);
  const [directionFilter, setDirectionFilter] = useState('all');
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<DirectionalAudio | null>(null);
  const [saving, setSaving] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<DirectionalAudio | null>(null);
  ...
  async function load() {
    setLoading(true);
    try {
      const data = await apiFetch<DirectionalAudio[]>('/api/admin/directional');
      setItems(data);
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  useEffect(() => {
    setFiltered(directionFilter === 'all' ? items : items.filter((i) => i.direction === directionFilter));
  }, [items, directionFilter]);

  function openCreate() {
    setEditing(null);
    form.reset({ direction: 'left', distance_trigger_m: 50, audio_url: '', label: '', transcript: '', is_active: true, sort_order: 0 });
    setDialogOpen(true);
  }

  function openEdit(item: DirectionalAudio) {
    setEditing(item);
    form.reset({
      direction: item.direction,
      distance_trigger_m: item.distance_trigger_m,
  ...
  async function onSubmit(values: FormData) {
    ...
    try {
      if (editing) {
        await apiFetch(`/api/admin/directional/${editing.id}`, {
          method: 'PUT',
          body: JSON.stringify(values),
        });
      } else {
        await apiFetch('/api/admin/directional', {
          method: 'POST',
          body: JSON.stringify(values),
        });
      }
  ...
  async function confirmDelete() {
    ...
    try {
      await apiFetch(`/api/admin/directional/${deleteTarget.id}`, { method: 'DELETE' });
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      fd.append('file', file);
      const result = await apiUpload<{ audioUrl: string }>('/api/admin/directional/upload', fd);
  ...
  return (
    <ManageLayout>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Directional Audio</h1>
        <Button onClick={openCreate}>+ Add Audio</Button>
      </div>

      <div className="flex items-center gap-3 mb-4">
        <span className="text-sm font-medium">Filter by direction:</span>
          ...
          <TableBody>
            {filtered.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell>{item.distance_trigger_m}</TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{editing ? 'Edit Directional Audio' : 'Add Directional Audio'}</DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <FormField control={form.control} name="distance_trigger_m" render={({ field }) => (
                  <FormItem>
                    <FormLabel>Distance Trigger (m)</FormLabel>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Directional Audio?</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete "{deleteTarget?.label ?? deleteTarget?.direction}". This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
